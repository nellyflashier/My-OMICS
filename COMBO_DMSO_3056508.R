...
output_dir = "C:/Users/nelly/Desktop/University of Glasgow/R/Further OMICS/P53_activation_AML/Combo_v_dmso"
...
de = read.table("DE_Combo_vs_DMSO.csv", header= TRUE, row.names=1,sep="\t")
...
#_______6. Plots________________________________________________________________
#_______6.1 Volcano Plot________________________________________________________
ggp = ggplot(master, aes(x=log2fold, y=mlog10p, color=direction))+
  geom_point()+
  labs(title="Combo Volcano Plot", x="log2(Fold Change)", y="-log10(p.adj)")+
  savanna_theme+
  xlim(c(-10,10))+
  ylim(c(0,210))+
  scale_colour_manual(values=direction_colors, name="Legend")+
  annotate("text", x=10,  y=200, label=paste(n_up),   color="black", fontface="bold", size=3, hjust=2)+
  annotate("text", x=-7, y=200, label=paste(n_down), color="black", fontface="bold", size=3, hjust=2)+
...
ggp = multi_boxplot(em_symbols, candidate_genes_down10, ss$SAMPLE_GROUP)
ggp = ggp + 
  scale_fill_manual(values = group_colors)+
...
ggp = make_heatmap(em_symbols, sig_genes[1:50])
ggp = ggp +
  savanna_theme
ggp
...
ggp = pathway_all$plots$ggp.barplot +
  labs(title = "Combo Top 10 Enriched Biological Processes", x = "Gene Count")+ savanna_theme
save_emf(ggp, "pathway_barplot.emf")

#Extract dot plot
ggp = pathway_all$plots$ggp.dotplot +
  labs(title = "Combo Top 10 Enriched Biological Processes")+ savanna_theme
save_emf(ggp, "pathway_dotplot.emf")

#Extract ontology genes
ggp=pathway_all$plots$ontology6$ggp.heatmap+
  labs(title = "DNA Replication")
ggp
save_emf(ggp, "DNA_rep_heatmap.emf")
...
ggp = pathway_up$plots$ggp.barplot +
  labs(title = "Combo Top 10 UPregulated Biological Processes", x = "Gene Count")+ savanna_theme
ggp
save_emf(ggp, "pathway_Up_barplot.emf")

# Extract dot plot
ggp = pathway_up$plots$ggp.dotplot +
  labs(title = "Combo Top 10 Upregulated Biological Processes")+savanna_theme
ggp
...
ggp = pathway_down$plots$ggp.barplot +
  labs(title = "Combo Top 10 Downregulated Biological Processes", x = "Gene Count")+ savanna_theme
ggp
save_emf(ggp, "pathway_down_barplot.emf")

# Extract dot plot
ggp = pathway_down$plots$ggp.dotplot +
  labs(title = "Combo Top 10 Downregulated Biological Processes")+savanna_theme
ggp
...
ggp = make_chr_plot(master[sig_genes,], "Chromosomal Distribution - COMBO vs DMSO")
ggp
save_emf(ggp, "chr_distribution_combo.emf")
