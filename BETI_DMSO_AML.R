...
de = read.table("DE_BETI_vs_DMSO.csv", header= TRUE, row.names=1,sep="\t")
annotations = read.table("anno.csv", header= TRUE, row.names=1,sep="\t")
ss = read.table("ss.csv", header= TRUE, row.names=1,sep="\t")

#subset to keep relevant groups
ss = ss[-c(7:12),,drop = FALSE ]
em = em[, -c(7:12)]
...
ggp = pathway_all$plots$ggp.barplot +
  labs(title = "BETi Top 10 Enriched Biological Processes", x = "Gene Count")+ savanna_theme
save_emf(ggp, "pathway_barplot.emf")

#Extract dot plot
ggp = pathway_all$plots$ggp.dotplot +
  labs(title = "BETi Top 10 Enriched Biological Processes")+ savanna_theme
save_emf(ggp, "pathway_dotplot.emf")

#Extract genes in DNA replication ontology
ggp = pathway_all$plots$ontology3$ggp.boxplot+
  scale_fill_manual(values = group_colors)+
  labs(title = "DNA Replication")
ggp
save_emf(ggp, "DNA_rep_genes.emf")
...
ggp = pathway_up$plots$ggp.barplot +
  labs(title = "BETi Top 10 UPregulated Biological Processes", x = "Gene Count")+ savanna_theme
save_emf(ggp, "pathway_Up_barplot.emf")

# Extract dot plot
ggp = pathway_up$plots$ggp.dotplot +
  labs(title = "BETi Top 10 Upregulated Biological Processes")+savanna_theme
...
ggp = pathway_down$plots$ggp.barplot +
  labs(title = "BETi Top 10 Downregulated Biological Processes", x = "Gene Count")+ savanna_theme
save_emf(ggp, "pathway_down_barplot.emf")

# Extract dot plot
ggp = pathway_down$plots$ggp.dotplot +
  labs(title = "BETi Top 10 Downregulated Biological Processes")+savanna_theme
save_emf(ggp, "pathway_down_dotplot.emf")


#_____6.4 chromosomal distribution of significant genes for each comparison
ggp = make_chr_plot(master[sig_genes,], "Chromosomal Distribution - BETi vs DMSO")
ggp
save_emf(ggp, "chr_distribution_beti.emf")
